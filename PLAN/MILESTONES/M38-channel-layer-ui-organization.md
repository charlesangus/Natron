...
- [x] M38.P9.T1a — Re-take the after-shots after Phase 38.10
  - files: `build/m38scout/gui.py`, `build/m38scout/run.sh`
...
- [x] M38.P9.T2 — Packaged release AppImage and user checkpoint (round 2)
...
- 2026-09-22 — **Phase 38.10 gate + 38.9.T1a green**: debug ctest 391/391, debug smoke passed, premult grep empty, all 11 after-shots retaken on `67ed048cc` (Blur shows the `[−]`/`[+]` column, divider and a `⌊ R G B` line under the `spec.*` row; Roto shows `Layer [mask] [A]`; Tracker has no layer knob), `dump-after.txt` regenerated (Tracker: none; Constant's `outputComponents` secret; Grade shows `unPremultBy`/`unPremultByChannel`). Xvfb checks for T1/T2/T3/T5/T8 pass programmatically (`build/m38p10/verify-*.txt`); T5's one-step undo and T6's red error frame were confirmed visually only (the headless harness could not drive Edit→Undo or read the graph's text items). Release AppImage at `build/release/artifacts/Natron-2.6.0-x86_64.AppImage` for the round-2 checkpoint. Harness note: the m38scout GUI scripts must run *inside* the container (`devshell.sh`), where PySide6 ships QtTest; a modal-opening combo pick has to be invoked with `Qt.QueuedConnection` or the timer chain never resumes.
...
- 2026-09-22 — **UAT round 2 passed** (user) on the packaged release AppImage. The round also produced one more fix, approved by the user with the checkpoint and committed as-is (`870af3a05`): the colour family's `(Un)premult by` channel choice now takes effect host-side — the node's source is divided by the selected channel before the plug-in renders and the result is multiplied back, replacing the plug-in's alpha-only pair.
- 2026-09-22 — **`fetch-assets.sh` fork-pin comments trimmed before merge** (user): the per-fork delta changelogs M38 grew there are change history; the script keeps one or two lines per fork plus a pointer to the fork PRs.
- 2026-09-22 — **PR #30 review round (Codex, 6 findings)**: fixed 4 in `80da4e06b` — one-channel planes were divided by `(Un)premult by` but never multiplied back (kernel tested pixel index 0 against logical bit 3), PyPlug export dropped regex exclusions and None channel selects, and an audit-history comment in a test. Declined 2: downstream components-needed staleness after a same-hash metadata change (not reachable — hashes propagate downstream, and the only same-hash change is a Read's decoder load before anything is connected), and exact Color subsets on Write (needs openfx-io work — WriteOIIO names one plane's channels by count). Debug ctest 396/396 ran green.
