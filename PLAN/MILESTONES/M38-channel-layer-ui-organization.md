...
- [ ] M38.P2.T6 — Delete `ImagePremultiplicationEnum` end to end; cache version 5→6
  ...
  - verify: `grep -rn -i "ImagePremultiplicationEnum\|getPremult\|outputPremult\|inputPremult" Engine Gui Global Tests` returns zero hits; full `ctest` green; a stale disk cache from a previous build is discarded on first launch (log line).
  - size: L
...
- [ ] M38.P3.T5 — Nodes create the knobs: eligibility, placement, quad adoption, mask selects, listing
