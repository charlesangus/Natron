...
- [ ] M38.P10.T3 — A layer can be chosen by only one layer row; regex rows do not consume layers
...
- [ ] M38.P10.T5 — GUI: indented `|-` button line under each regex row, union of matched channels
...
- [ ] M38.P10.T6 — Node summary names the matched layers, not the pattern
...
- [ ] M38.P10.T7 — Layer users: RotoPaint's internal tree does not count (UAT step 2.6)
...
- [ ] M38.P10.T8 — Write's channel combo lists the present layers without a click-select-click dance (UAT step 3)
...
- [ ] M38.P10.T9 — Write All: a single-channel user layer comes out as `mask.A`, never `subimage03` (UAT step 3, openfx-io fork)
...
- [ ] M38.P10.T10 — Restore `(Un)premult by` on the colour family as `unPremultBy` / `unPremultByChannel` (UAT step 4, openfx-misc fork)
...
- [ ] M38.P10.T11 — Mask channel absent from a connected Mask input → error state, render fails (UAT step 6.1)
...
- [ ] M38.P10.T13 — Generators: the layer select drives a hidden `outputComponents` (UAT step 6.3)
...
- [ ] M38.P10.T14 — Read format: a Read's output format is its file's, whatever the project format (UAT incidental)
