...
- [ ] M61.P3.T2 — An implicit read of a missing layer fails the render, like an explicit row does
  - files: `Engine/Nodes/Channel/Shuffle.cpp`, `Tests/ShuffleRender_Test.cpp`, `Tests/Shuffle_Test.cpp` (only if an assertion there depends on the old rule)
  - approach: `checkExtraChannelsPresent` (~`Shuffle.cpp:875`) checks explicit mapping rows only, per M34's design, so `In 1 = diffuse` feeding `Out 1` through the implicit default renders 0 silently on a frame without `diffuse`. Extend it to check every output channel that the render actually produces, using `getEffectiveSource(outSlot, c, time, view)`, not just `mapping->getRows()`. Fail when an effective `eInput` source's slot layer is not among the input's `getPresentLayers(time, view, …)`, reusing the existing message format and persistent-error/clear path.
    - The error is only for a missing layer. An implicit channel that is beyond a present layer's channels still renders 0 silently. `getEffectiveSource` already zeroes it, and ShuffleCopy's default `out1.A ← in1.A` on an RGB-only Color must not start erroring.
    - A None slot, a disconnected input and an unread slot stay silent, as they do now.
    - Check that the `isIdentity` call at ~`Shuffle.cpp:597` still behaves: in1 = out1 = Color on a normal input stays identity.
  - verify: `ImplicitDefaultOnAMissingLayerRendersZeroSilently` is renamed and inverted: the render fails with the persistent error naming `diffuse`. `ImplicitAlphaOfAnRgbOnlyColorRendersZeroSilently` stays green, unchanged. A new time-varying case on graph (a), with `In 1 = diffuse`, `Out 1 = Color` and no explicit row, errors on frame 2 and clears on frame 1. `ctest -R 'Shuffle|TimeVaryingLayers'` is green, and so is the full debug ctest.
  - size: M
...
- 2026-09-26 — **UAT round 2: menus pass; an implicit read of a missing layer now errors too** (user). On `M61-0ed1db2ee.AppImage` the layer menus relist correctly. The Shuffle errored on an invalid In 2 but not on an invalid In 1. The difference wasn't the slot: the UAT's In 2 step uses an explicit mapping row, while In 1 feeds Out 1 through the implicit default, and M34 made `checkExtraChannelsPresent` check explicit rows only (M34.P6 test (f)). The user wants both cases to error. This reverses that M34 rule for a **missing layer** only. An implicit channel beyond a present layer's count (for example ShuffleCopy's default `A ← in1.A` on an RGB input) stays a silent 0. Added as P3.T2. After it lands, P4.T1 repackages the AppImage and adds an In 1/implicit step to `M61-uat.md` for the next UAT round.
